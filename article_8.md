...
取整为1000mm，此时操作气速分别为$4\times0.519/(\pi*1^2)=0.661\mathrm{m/s}$，$4\times0.494/(\pi*1^2)=0.629\mathrm{m/s}$。塔横截面积为$0.78\mathrm{m^2}$。
...
采用单溢流型的平顶弓形溢流堰、弓形降液管、平行受液盘，且不设进口内堰。
   ...
   于是堰上溢流高度$h_{ow}=0.00284\timesE\left(\frac{L_R}{l_w}\right)^{\frac{2}{3}}=0.00284\times{1.01,1.03}\times({2.196,10.394}/0.7)^{\frac{2}{3}}={0.06147,0.01767}\mathrm{m}\geq 0.006\mathrm{m}$
