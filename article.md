...
对于塔底，压力为$105.3+0.7\times20=119.3 \mathrm{kPa}$,因此精馏段平均压力为$(110.9+119.3)/2=115.1 \mathrm{kPa}$。
...
Print[rhoMix[84.26,(0.987+0.728)/2],rhoMix[84.26,(0.987+0.728)/2]]
```

可以得到，精馏段平均密度为$843.9\mathrm{kg/m^3}$，提馏段平均密度为$924.0\mathrm{kg/m^3}$
...
采用单溢流型的平顶弓形溢流堰、弓形降液管、平行受液盘，且不设进口内堰。
   ...
   2. 出口堰高$h_W$
   由$l_w/D=0.7$，$L_R/l_w^{2.5}=2.196/0.63^{2.5}=6.97$，查表[^chart2]可得液流收缩系数$E=1.02$.
   ...
   3. 降液管宽度与面积
   由$l_w/D=0.7$，查表[^chart3]可得$W_d/D=0.14$，$A_f/A_T=0.087$，其中$A_T$为塔内空间的横截面积.

   [^chart3]:管国锋等.化工原理（第四版）[M].化学工业出版社.354：图8-17
